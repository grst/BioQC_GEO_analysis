...
```{r, echo=FALSE, fig.cap="Entitiy relationship diagram of the *BioQC* database scheme. Click the [here](https://github.com/grst/BioQC_GEO_analysis/raw/master/db/design/er_diagram.pdf) for an enlarged version. Greenish tables are imported from GEOmetadb. Yellowish tables are additional tables designed for this study. Three dots (...) indicate columns from GEOmetadb which are omitted in the visualisation because they are not relevant for this study."}
...
To perform a preliminary quality control on each study we calculated the quartiles and the mean of the expression values 
of each study in GEO and import these values into the database: 
