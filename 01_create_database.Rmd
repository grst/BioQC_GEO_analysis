...
```{r include=FALSE, fig.cap="Entitiy relationship diagram of the *BioQC* database scheme. Click the [here] for an enlarged version. Greenish tables are imported from GEOmetadb. Yellowish tables are additional tables designed for this study. Three dots (...) indicate columns from GEOmetadb which are omitted in the visualisation because they are not relevant for this study."}
knitr::include_graphics("db/er_diagram_test.png")
```
