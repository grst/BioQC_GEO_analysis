...
gmt.table = do.call(rbind.data.frame, gmt.list)
...
tissue.raw = dbGetQuery(mydb, "select gsm, characteristics_ch1 from bioqc_gsm where characteristics_ch1 like '%tissue: %'")
tissues.extracted = lapply(tissue.raw$CHARACTERISTICS_CH1, extractTissue)
tissues = lapply(tissues.extracted, sanitizeTissue)
dbSendUpdate(mydb, "create table bioqc_tmp_gsm(gsm varchar2(10) primary key, tissue varchar2(80), tissue_orig clob)")
table = cbind(gsm=tissue.raw$GSM, tissue=tissues, tissue_orig=tissues.extracted)
dbAppendDf("BIOQC_TMP_GSM", table)
sqlUpdateGsm = "
  update bioqc_gsm b
  set (tissue, tissue_orig) = ( SELECT tissue, tissue_orig from bioqc_tmp_gsm t
                                where b.gsm = t.gsm)"
