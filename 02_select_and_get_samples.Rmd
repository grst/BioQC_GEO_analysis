...
## Exclude duplicates (samples that are part of multiple studies)
samples references in multiple gse are indeed indentical:
```{r}
sql = "with duplicates as (select /*+ parallel(16) */ gsm, count(distinct gse) from bioqc_res group by gsm having count(distinct gse) > 1)
select /*+ parallel(16) */ * from bioqc_res join duplicates on duplicates.gsm = bioqc_res.gsm order by signature, bioqc_res.gsm"
```
