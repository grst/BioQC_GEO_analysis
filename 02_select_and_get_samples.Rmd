...
create or replace temp view has_annot as (
    select gsm.gsm, gse_gsm.gse from gsm
    join gse_gsm on gse_gsm.gsm = gsm.gsm
    where gpl in (select distinct gpl from bioqc_gpl_annot) 
);
create or replace temp view tissue_and_annot_pkg as (
    select * from tissue_annotated 
    intersect select * from annotation_package
);
create or replace temp view tissue_and_gpl_annot as (
    select * from tissue_annotated 
    intersect select * from has_annot
);
create or replace temp view tissue_and_annotation as (
    select * from tissue_annotated 
    intersect (select * from annotation_package union select * from has_annot)
);"
...
union select 'tissue and annotation package', count(gsm), count(distinct gse) from tissue_and_annot_pkg
union select 'tissue and annot_gpl', count(gsm), count(distinct gse) from tissue_and_gpl_annot
union select 'tissue and annot', count(gsm), count(distinct gse) from tissue_and_annotation
"
