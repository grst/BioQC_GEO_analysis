...
We can do this entirely in sql. We first need to create a materialized view for performance reasons. 
This is done with the query `db/queries/mk_extended_bioqc_res_table2.sql`. The cut-offs can be specified there. 

Second, we retrieve the information we need for the migration chart from that temporary table. 
```{r} 
query = readLines(file("db/queries/get_migration_chart.sql"))
migration = dbGetQuery(mydb, paste(query, collapse="\n"))
