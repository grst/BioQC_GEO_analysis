...
[BioQC](https://accio.gitub.io/BioQC) implements a computationally efficient Wilcoxon-Mann-Whitney (WMW) test which is applied to gene expression data on a sample-by-sample basis. Using BioQC, we can efficiently test for the enrichment of certain gene sets, or in this case *tissue signatures*. These tissue signatures are lists of genes, which are predominantly expressed in a certain tissue. If a signature yields a small p-value in the WMW-test we can therefore conclude that the respective tissue is present in the sample. 
...
Finally, we identified contamined samples from the signature scores. If a signature scores high, which is not associated with the annotated tissue, we assume the sample being heterogenous. In section [Tissue Migration](#tissue-migration), we describe how we identify contamined samples and discuss common pattern of tissue heterogenity. 
