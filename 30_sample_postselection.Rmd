...
To some of the studies in GEO a per-gene normalization has been applied. These studies cannot be analysed with BioQC as the ranking information of genes within one sample is lost. We identify normalized studies by the median gene expression.
...
hist(sm[sm > -5 & sm < 30],
     breaks=seq(-5, 30, .2), 
     main="Histogram of median gene expression per study",
     xlab="median gene expression (only values between -5 and 30)")
minor.tick(nx=5)
```

We observe three major peaks: The peaks at 0 and 1 represent the studies that have been normalized to 0 and 1 respectively. The majority of all other studies appear to have a median gene expression between 2 and 10.

We therefore decided to limit our analysis to studies with a median gene expression between 3 and 9 which equals approximately 2 standard deviations of the gaussian fitted to the third peak. 
